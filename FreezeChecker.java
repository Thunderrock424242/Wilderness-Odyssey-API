    ...
    private static class Watchdog implements Runnable {
        ...
        private void attemptGracefulShutdown(long freezeDurationMs) {
            try {
                ...
                CrashReportCategory category = crashReport.addCategory("FreezeChecker Info");
                category.setDetail("Freeze Duration (ms)", () -> String.valueOf(freezeDurationMs));
                category.setDetail("Threshold (ms)", () -> String.valueOf(FREEZE_TIMEOUT_MS));
                category.setDetail("Consecutive Checks Reached", () -> String.valueOf(MAX_FREEZE_CHECKS));
                ...
                try {
                    String friendlyReport = crashReport.getDetails();
                    String jsonReport = "{ \"crash_report\": " + escapeJson(friendlyReport) + " }";
                    Files.writeString(crashFile.toPath(), jsonReport);
                    Files.writeString(crashFile.toPath(), friendlyReport);
                    LOGGER.error("FreezeChecker: Crash report saved to {}", crashFile.getAbsolutePath());
                } catch (IOException ioException) {
                    LOGGER.error("Failed to write crash report to file!", ioException);
                }
