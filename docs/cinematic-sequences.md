...
Stage and cue timing are part of the network contract between a server definition and its client presentation. Changing that wire shape requires a network-version change. Stage durations remain server authoritative, while smooth per-frame values are derived from synchronized level game time without per-tick packets. Narration payloads carry the authoritative cue start time as well as the measured duration, so a delayed packet advances the subtitle and seeks into the WAV instead of restarting either one late.
...
The normal timeline lasts 1,776 ticks (88.8 seconds). The first 71.3 seconds remain locked; the final 17.5 seconds return movement for the briefing. The timing follows the measured conversational recordings without accelerating, truncating, or overlapping them:
...
| 136 | Medical diagnostic | Patient acquisition, contamination, and filtration status |
| 406 | Revival protocol | Rewarming, cryoprotectant washout, and vasopressor response |
| 706 | Cardiac pacing | Direct countdown, electrical pacing jolt, and restored-pulse confirmation |
| 874 | Suspension drain | Independent-breathing confirmation, contaminated-fluid drain, and mist burst |
| 1,076 | Black transition | Exterior view cuts to black before first person |
| 1,096 | Eyes reopening | Lower occupant-relative camera, eyelids, strong blur, and consciousness report |
| 1,196 | Mask release | Breathing-mask release cue and mechanical sound |
| 1,286 | Tube opening | Opening machinery cue while blur and mechanical shake ease |
| 1,346 | Balance check | Camera levels and A.E.T.H.E.R directly tells the player to remain still |
| 1,426 | Recovery walk | Controls return for a seventeen-and-a-half-second, movement-aware briefing |
...
Blur uses Minecraft's native transient blur pass and respects the screen-effect accessibility scale. Eyelids, contaminated-fluid tint, diagnostic scan, warning wash, condensation, pacing flash, subtitles, FOV, shake, and blur are presentation-only; they do not mutate player rotation or facility blocks. Camera motion, tube animation, the diagnostic sweep, cardiac waveform, subtitles, and authored voice all derive from the same synchronized sequence, stage, or cue clock. An A.E.T.H.E.R medical panel shows a deliberately fictional but internally consistent recovery trend for estimated core temperature, heart rate, oxygen saturation, cardiac waveform, and current protocol state. The HUD deliberately uses one restrained, asymmetrical patient-monitor panel and one caption plate; it avoids full-screen decorative scanlines, floating status banners, and individually boxed subtitle lines. These display values are cinematic telemetry, not a general medical simulation.

## A.E.T.H.E.R narration

All medical and recovery lines are authored cue ids. They do not call Ollama and cannot invent a procedure, observation, or capability. The cryo awakening packages fixed offline 24 kHz PCM WAV clips generated by Kokoro-82M with the grounded `af_bella` voice at normal 1.0 speed. Ordinary contractions, short complete sentences, and direct instructions replace the previous ellipses and meditative pauses. The performance is warm and controlled without whisper, radio distortion, or synthetic ambience. This is an original A.E.T.H.E.R direction informed by broad caretaker-fiction qualities, not an imitation of another game's actor. The client preloads the bounded assets during the black-screen lead-in and plays them through the existing A.E.T.H.E.R JavaSound owner, bypassing live synthesis latency and Windows Narrator. Subtitle durations are the measured clip lengths plus a short fade tail, and the stage cue sheet has a regression test that rejects overlaps. Text, audio, and medical effects share the server-authored cue boundary. If delivery is delayed, subtitle fading catches up to that boundary and WAV playback seeks past the missed portion, including local audio queue and clip-setup time. Cleanup stops the active clip. Set `aether_voice.cinematicNarration` to false in the client config to silence authored cinematic speech while retaining subtitles. The optional local Kokoro service now also defaults ordinary A.E.T.H.E.R conversation to `af_bella` at normal speed; radio processing defaults off.

The checked-in generators are `tools/aether_voice_service/generate_cryo_voice.ps1` and `generate_cryo_voice.py`. They read the authoritative English translation strings, use the separately cached Apache-licensed Kokoro model, regenerate only the cryo voice directory, apply restrained click-free mastering without a synthetic filter, and write a manifest containing the exact source text and measured PCM duration for every clip. The asset test rejects translation drift, unexpected WAV formats, measured-duration drift, and disagreement with the sequence cue sheet. The first deliberate authoring run uses `-AllowModelDownloads`; later runs are cache-only. Regenerating clips after text changes is deliberate source authoring and is never performed at game runtime. The packaged mod requires neither Python nor the model cache.
...
Use a development client and verify both automatic first login and the developer command. During the first 71.3 seconds, attempt walking, jumping, sprinting, attacking, mining, using the tube, opening inventory, and changing perspective. Confirm none disrupt the sequence; chat and the pause menu intentionally remain available. Confirm the original cryo-tube shape and UV layout are unchanged and the opening exterior view sees the softly lit local-player proxy, breathing mask, suspension volume, bubbles, diagnostic sweep, and life-support routing inside it rather than the tube ceiling; the black cut then enters the lower first-person view. Check that every subtitle begins with its matching grounded conversational clip, no line overlaps the next, the pacing flash lands immediately after the spoken final count, the restored-pulse and independent-breathing confirmations play before draining, the contaminated-fluid screen tint clears, blur fades, and controls return for the walking briefing. Watch the patient monitor at several GUI scales and window sizes: it should remain one quiet, readable instrument panel, subtitles should occupy a single caption plate without covering the monitor, and the completion objective should use the same visual language. The restored static model is not expected to animate its door, fluid, or conduits; those cinematic details are temporary renderer layers.
